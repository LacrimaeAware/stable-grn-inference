...
# ---------------------------------------------------------------------------
# Linear response / deconvolution model (experiment 23).
#
# Simplified causal-propagation model: a sparse DIRECT operator W generates the
# observed TOTAL response by propagation,
#     D = (I - W)^{-1} - I = W + W^2 + W^3 + ...
# (direct + indirect + ... effects). The inverse recovers the direct operator
# exactly in the noiseless case:
#     W = I - (I + D)^{-1} = D (I + D)^{-1}.
# These are the "solve the flow field backward for the stick" tools.
# ---------------------------------------------------------------------------


def make_sparse_dag(n_genes: int, density: float, *, weight_scale: float = 0.4,
                    spectral_radius: float = 0.7, seed: int = 0, dag: bool = True) -> np.ndarray:
    """Random sparse directed operator W with no self-edges. If ``dag`` (default) the
    edges are upper-triangular (acyclic, always invertible). Otherwise a general sparse
    matrix is rescaled to the given spectral radius (< 1) so (I - W) is invertible."""
    rng = np.random.default_rng(seed)
    mask = rng.random((n_genes, n_genes)) < density
    np.fill_diagonal(mask, False)
    if dag:
        mask = np.triu(mask, 1)
    W = mask * rng.normal(0.0, weight_scale, size=(n_genes, n_genes))
    if not dag:
...
def propagation_forward(W: np.ndarray) -> np.ndarray:
    """Total response D = (I - W)^{-1} - I generated by a direct operator W."""
    n = W.shape[0]
    eye = np.eye(n)
    return np.linalg.inv(eye - W) - eye


def deconvolve_response(D: np.ndarray, *, method: str = "ridge", lam: float = 0.1,
                        rcond: float = 1e-3) -> np.ndarray:
    """Recover a direct-effect operator W from a total response D.

    Exact (noiseless) inverse is ``W = I - (I + D)^{-1}``. ``method='ridge'`` adds
    ``lam`` to the diagonal before inverting (stabilizes a near-singular I+D);
    ``method='pinv'`` uses a truncated pseudoinverse with ``rcond``. lam=0 / tiny rcond
    reproduce the exact inverse on clean data."""
    n = D.shape[0]
    eye = np.eye(n)
    M = eye + D
    if method == "ridge":
        W = eye - np.linalg.solve(M + lam * eye, eye)
    elif method == "pinv":
        W = eye - np.linalg.pinv(M, rcond=rcond)
    else:
        raise ValueError(f"unknown method {method!r}")
    return W


def operator_edges(W: np.ndarray, labels: list[str]) -> pd.DataFrame:
    """Directed off-diagonal edges (source, target, score=|W[s,t]|, signed=W[s,t])."""
    rows = []
    for i, s in enumerate(labels):
        for j, t in enumerate(labels):
            if i != j:
                rows.append((s, t, abs(float(W[i, j])), float(W[i, j])))
    return pd.DataFrame(rows, columns=["source", "target", "score", "signed"])
