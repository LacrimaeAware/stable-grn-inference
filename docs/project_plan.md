...
This public plan summarizes the intended progression without depending on private planning notes.
...
- Ran a rigorous diagnostics experiment (experiment 17) that adjudicates the original stability-selection thesis, with paired-over-networks bootstrap CIs for n=5. Findings: (a) the error is SKELETON detection, not orientation (orientation-given-skeleton ~0.81-0.96 vs 0.50 for a symmetric control; small undirected-vs-directed gap; reciprocal FPs are false pairs, not mis-oriented true edges); (b) a theory / square-root LASSO penalty (~sqrt(log p / n)) matches or beats the grid oracle at Size100, so alpha is sample-complexity-predictable; (c) fusion's Size100 gain is genuine complementarity (cross-method beats within-method-bootstrap, CI excludes 0), absent at Size10; (d) formal stability selection (Meinshausen-Buhlmann bound, trajectory subsampling) does NOT support the strong thesis: the bound is too loose at p>>n and the selection-probability ranking underperforms a single tuned fit. The original stability thesis is therefore tested-and-not-supported in its strong form; the reframed thesis is skeleton/sample-complexity-limited inference with a theory-predictable penalty and complementary-evidence fusion.
...
- A separate pilot on structured representation learning is possible later. Kymatio is installed for 1D wavelet scattering (use `stable_grn_inference._compat.ensure_scipy_sph_harm()` before importing `kymatio.numpy` on SciPy >= 1.15).
