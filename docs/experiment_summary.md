...
- The essentiality and cascade-position experiment (experiment 26) uses the dominant cascade signal instead of removing it. Ranking the 651 perturbed genes by four data-derived measures (knockout-response magnitude, cascade-axis projection, response breadth, and control-cell correlation centrality) gives a reproducible essentiality axis: the three interventional measures agree (Spearman 0.79 to 0.89), the ranking is split-half reproducible at 0.97, it anti-correlates with the number of surviving cells per perturbation (-0.35, consistent with essential knockouts being depleted), and it recovers recognizable essential machinery (ribosomal proteins, spliceosome, proteasome, nuclear pore, translation initiation). The observational correlation centrality is independent of the interventional measures (0.12 to 0.26), consistent with the earlier observational-vs-interventional gap. A net-effect ordering (net_out: how much perturbing a gene moves others minus how much they move it) is highly reproducible (0.986) and separates upstream information-processing genes (translation initiation, splicing, proteasome, MTOR) from downstream cell-cycle and structural effectors (cyclins, tubulins, histones, replication); it is moderately correlated with essentiality (0.46) but distinct. Both axes partly recover known biology, so this validates that the data and measures behave sensibly; external validation against annotated essentiality (DepMap) is the next step.
- The cascade-adjacent edges experiment (experiment 27) tests whether genes adjacent in the cascade ordering (experiment 26) give more direct edges, on the hypothesis that direct edges connect ordering-adjacent genes while the cascade connects distant ones. The hypothesis is not supported on RPE1: the Spearman correlation between ordering distance and the mediation ratio (strongest 2-step path through any gene divided by the direct effect) is -0.06, the mean mediation ratio is near 3.3 for both nearest- and farthest-quartile pairs (almost every pair is well explained by a 2-step path regardless of ordering position), and restricting the top-200 edges to ordering-local pairs lowers split-half reproducibility to 0.43 versus 0.81 for the raw total-effect ranking and 0.91 for the observational correlation ranking. The convergent cascade provides a strong indirect path for nearly every pair, so ordering position does not separate direct from cascade-mediated effects. The observational correlation ranking being the most reproducible is consistent with correlation remaining a strong baseline throughout.
- The separability phase diagram (experiment 28) steps off RPE1's unknowable-truth data and studies the dominant-mode-versus-specific-structure separation on synthetic systems with known ground truth. A response matrix is generated by mixing a known sparse-operator response (the experiment-23 propagation model) with a rank-1 dominant mode and i.i.d. noise, controlled by two knobs: rho (dominant-mode variance fraction; RPE1 measures ~0.53) and the specific-SNR. Sweeping both separates two failure axes the project had conflated. The dominant-mode fraction rho kills naive recovery but is fixable: deflating a clean rank-1 mode makes recovery essentially rho-invariant. The specific-SNR is the other axis and is not fixable: below a noise floor (~0.2 here) no method (raw, deflation, program subtraction, ridge inverse) clears chance at any rho. RPE1 sits at high rho with low specific-SNR, so removing the cell-cycle mode does not help because little recoverable specific signal remains underneath it — which restates the experiment 21/22/23/24/25/27 negatives as one point on the diagram rather than separate failures. The transferable result is the shape of the boundary (rho fixable, specific-SNR the binding floor) and a data-selection criterion: a dataset needs specific-SNR above the floor, not merely a low dominant-mode fraction. It is a diagnostic, not a deployable method; the synthetic dominant mode is a clean rank-1 component, so deflation's success there is a property of the model, not a claim about real data.
